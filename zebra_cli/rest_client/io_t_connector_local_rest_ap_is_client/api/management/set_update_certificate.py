from http import HTTPStatus
from typing import Any, Optional, Union, cast

import httpx

from ...client import AuthenticatedClient, Client
from ...types import Response, UNSET
from ... import errors

from ...models.error import Error
from ...models.set_update_certificate import SetUpdateCertificate
from typing import cast



def _get_kwargs(
    certificates: str,
    *,
    body: SetUpdateCertificate,

) -> dict[str, Any]:
    headers: dict[str, Any] = {}


    

    

    _kwargs: dict[str, Any] = {
        "method": "put",
        "url": "/cloud/certificates/{certificates}".format(certificates=certificates,),
    }

    _kwargs["json"] = body.to_dict()


    headers["Content-Type"] = "application/json"

    _kwargs["headers"] = headers
    return _kwargs


def _parse_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Optional[Union[Any, Error]]:
    if response.status_code == 200:
        response_200 = cast(Any, None)
        return response_200
    if response.status_code == 500:
        response_500 = Error.from_dict(response.json())



        return response_500
    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Response[Union[Any, Error]]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    certificates: str,
    *,
    client: Union[AuthenticatedClient, Client],
    body: SetUpdateCertificate,

) -> Response[Union[Any, Error]]:
    """ Install Certificate

     Install Certificate

    Args:
        certificates (str):
        body (SetUpdateCertificate): Install certificate

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[Any, Error]]
     """


    kwargs = _get_kwargs(
        certificates=certificates,
body=body,

    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)

def sync(
    certificates: str,
    *,
    client: Union[AuthenticatedClient, Client],
    body: SetUpdateCertificate,

) -> Optional[Union[Any, Error]]:
    """ Install Certificate

     Install Certificate

    Args:
        certificates (str):
        body (SetUpdateCertificate): Install certificate

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[Any, Error]
     """


    return sync_detailed(
        certificates=certificates,
client=client,
body=body,

    ).parsed

async def asyncio_detailed(
    certificates: str,
    *,
    client: Union[AuthenticatedClient, Client],
    body: SetUpdateCertificate,

) -> Response[Union[Any, Error]]:
    """ Install Certificate

     Install Certificate

    Args:
        certificates (str):
        body (SetUpdateCertificate): Install certificate

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[Any, Error]]
     """


    kwargs = _get_kwargs(
        certificates=certificates,
body=body,

    )

    response = await client.get_async_httpx_client().request(
        **kwargs
    )

    return _build_response(client=client, response=response)

async def asyncio(
    certificates: str,
    *,
    client: Union[AuthenticatedClient, Client],
    body: SetUpdateCertificate,

) -> Optional[Union[Any, Error]]:
    """ Install Certificate

     Install Certificate

    Args:
        certificates (str):
        body (SetUpdateCertificate): Install certificate

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[Any, Error]
     """


    return (await asyncio_detailed(
        certificates=certificates,
client=client,
body=body,

    )).parsed
