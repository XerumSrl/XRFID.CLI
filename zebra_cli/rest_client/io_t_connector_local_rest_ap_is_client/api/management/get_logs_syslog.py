from http import HTTPStatus
from typing import Any, Optional, Union, cast

import httpx

from ...client import AuthenticatedClient, Client
from ...types import Response, UNSET
from ... import errors

from ...models.error import Error
from ...models.get_logs_syslog_log_type import check_get_logs_syslog_log_type
from ...models.get_logs_syslog_log_type import GetLogsSyslogLogType
from typing import cast



def _get_kwargs(
    log_type: GetLogsSyslogLogType,

) -> dict[str, Any]:
    

    

    

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/cloud/logs/{log_type}".format(log_type=log_type,),
    }


    return _kwargs


def _parse_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Optional[Error]:
    if response.status_code == 400:
        response_400 = Error.from_dict(response.json())



        return response_400
    if response.status_code == 500:
        response_500 = Error.from_dict(response.json())



        return response_500
    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Response[Error]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    log_type: GetLogsSyslogLogType,
    *,
    client: Union[AuthenticatedClient, Client],

) -> Response[Error]:
    """ Retrieve Log

     Retrieves a reader log

    Args:
        log_type (GetLogsSyslogLogType):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Error]
     """


    kwargs = _get_kwargs(
        log_type=log_type,

    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)

def sync(
    log_type: GetLogsSyslogLogType,
    *,
    client: Union[AuthenticatedClient, Client],

) -> Optional[Error]:
    """ Retrieve Log

     Retrieves a reader log

    Args:
        log_type (GetLogsSyslogLogType):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Error
     """


    return sync_detailed(
        log_type=log_type,
client=client,

    ).parsed

async def asyncio_detailed(
    log_type: GetLogsSyslogLogType,
    *,
    client: Union[AuthenticatedClient, Client],

) -> Response[Error]:
    """ Retrieve Log

     Retrieves a reader log

    Args:
        log_type (GetLogsSyslogLogType):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Error]
     """


    kwargs = _get_kwargs(
        log_type=log_type,

    )

    response = await client.get_async_httpx_client().request(
        **kwargs
    )

    return _build_response(client=client, response=response)

async def asyncio(
    log_type: GetLogsSyslogLogType,
    *,
    client: Union[AuthenticatedClient, Client],

) -> Optional[Error]:
    """ Retrieve Log

     Retrieves a reader log

    Args:
        log_type (GetLogsSyslogLogType):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Error
     """


    return (await asyncio_detailed(
        log_type=log_type,
client=client,

    )).parsed
